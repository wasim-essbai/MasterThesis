...
%     plot(BP_original);
    ...
    [sys_pk,sys_loc]= findpeaks(PPG,'MinPeakProminence', max(PPG)/15); % max values of PPG signal
    [dias_pk,dias_loc]=findpeaks(-PPG,'MinPeakProminence', max(-PPG)/15); % min values of PPG signal
    dias_pk = -dias_pk;
